...
var (
	...
	flagClip = flag.Bool("clip", false, "copy the code to the clipboard")
)

func usage() {
	fmt.Fprintf(os.Stderr, "usage:\n")
	fmt.Fprintf(os.Stderr, "\tsernex-2fa -add [-7] [-8] [-hotp] keyname\n")
	fmt.Fprintf(os.Stderr, "\tsernex-2fa -list\n")
	fmt.Fprintf(os.Stderr, "\tsernex-2fa [-clip] keyname\n")
	os.Exit(2)
}

func main() {
	log.SetPrefix("sernex-2fa: ")
	log.SetFlags(0)
	flag.Usage = usage
	flag.Parse()

	k := readKeychain(filepath.Join(os.Getenv("HOME"), ".sernex-2fa-data"))
...
func readKeychain(file string) *Keychain {
	...
	for i, line := range lines {
		...
		log.Printf("%s:%d: the key is a mal-formed key", c.file, lineno)
	}
	return c
}
...
func (c *Keychain) add(name string) {
	...
	if *flag7 {
		size = 7
		if *flag8 {
			log.Fatalf("cannot use both 7 digit and 8 digit code together")
		}
	} else if *flag8 {
		size = 8
	}
